...
`body::before` (see `src/index.css`) paints `var(--scene)` fixed, full-viewport, behind everything. Pages must stay transparent — **never put `bg-background` on a page wrapper**, or the scene disappears.

Scenes live in `public/backgrounds/scene-day.svg` and `scene-night.svg`, generated by `generate-scene.mjs` in this folder (same seed = same geometry in both):

```bash
node .claude/skills/tugasku-design/generate-scene.mjs public/backgrounds
```

To use a different background entirely, drop an image in `public/backgrounds/` and point `--scene` at it in `:root` (night) and `.light` (day).
